# niocortex_saas/pedagogical/views.py
...
def adicionar_aluno(request, turma_id):
    ...
    return render(request, 'pedagogical/alunos/form.html', {'form': form, 'turma': turma, 'titulo': 'Novo Aluno'})

@login_required
def editar_aluno(request, aluno_id):
    # Busca o aluno garantindo que pertence ao tenant do usuário
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Redireciona para a turma deste aluno
            return redirect('pedagogical:detalhe_turma', turma_id=aluno.turma.id)
    else:
...
def excluir_aluno(request, aluno_id):
    aluno = get_object_or_404(Aluno, id=aluno_id, tenant_id=request.user.tenant_id)
    turma_id = aluno.turma.id # Guarda o ID para redirecionar depois
    ...
    # Reutiliza o template de confirmação genérico ou cria um específico
    return render(request, 'pedagogical/turmas/confirmar_exclusao.html', {'objeto': aluno, 'titulo': 'Excluir Aluno'})