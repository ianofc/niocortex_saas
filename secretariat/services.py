...
class SecretariatService:
    ...
    def emitir_documento(cls, user: CustomUser, aluno_id: str, modelo_id: str) -> DocumentoEmitido:
        ...
        # 1. Preparar o Contexto (Dados disponíveis para o template)
        # Aqui mapeamos o que o utilizador pode usar no editor (ex: {{ aluno.nome }})
        context_data = {
            'aluno': aluno,
            'escola': aluno.escola if hasattr(aluno, 'escola') else "NioCortex School",
            'data_hoje': "HOJE" # Substituir por datetime formatado
        }

        # 2. Renderizar o Conteúdo (Engine do Django)
        # Usamos o motor de templates do Django para fazer a substituição segura
