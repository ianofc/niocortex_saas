...
from django.http import JsonResponse
def api_zios_chat(request): return JsonResponse({'reply': 'Olá, sou o ZIOS!'})

# [AUTO-INJECTED] View para finalizar cadastro via ZIOS
...
def api_finalize(request):
    try:
        ...
        user_data = data.get('dados', {})
        
        # Extração básica (O ZIOS deve enviar isso estruturado)
        # Por enquanto, criamos um user genérico se vier vazio
        User = get_user_model()
        
        username = f"novo.user.{random.randint(1000,9999)}"
        matricula = f"{date.today().year}99{random.randint(1000,9999)}"
