...
from ..services.zios_register import chat_com_zios
from core.utils import gerar_matricula_zios, gerar_email_institucional
...
def register_view(request):
    if request.user.is_authenticated:
        ...
        return redirect('yourlife_social:home')
    return render(request, 'social/auth/register_zios.html')
...
def api_zios_chat(request):
    try:
        ...
        # Chama a IA
        resposta_ai = chat_com_zios(historico)
...
def api_finalize_registration(request):
    try:
        ...
        matricula = gerar_matricula_zios()
