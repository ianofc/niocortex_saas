...
class Funcionario(models.Model):
    ...
    Vincula-se ao User para permitir login, mas existe mesmo se o funcionário não acessar o sistema (ex: Jardineiro).
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    tenant_id = models.UUIDField(editable=False, db_index=True)
    
    # Vínculo com usuário de sistema (Opcional, pois nem todo funcionário loga)
    ...
    cpf = models.CharField(max_length=14, unique=True) # Validar unicidade por tenant no form/service
    ...
    # Documentação (Futuro: Uploads de PDF)
